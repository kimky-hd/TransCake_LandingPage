<%@ page contentType="text/html;charset=UTF-8" language="java" %>
    ...
    <div id="authModal" class="fixed inset-0 z-[100] hidden flex items-center justify-center pointer-events-auto">
        <!-- Backdrop -->
        <div id="authModalBackdrop" class="absolute inset-0 bg-black/40 backdrop-blur-md transition-opacity opacity-0">
        </div>

        <!-- Modal Container -->
        <div id="authModalContent"
            class="relative bg-white rounded-3xl shadow-2xl overflow-hidden max-w-4xl w-full mx-4 flex flex-col md:flex-row transform scale-95 opacity-0 transition-all duration-300 md:h-[640px]">

            <!-- Close Button -->
            <button id="closeAuthModalBtn"
                class="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-white/50 backdrop-blur-sm text-slate-800 hover:bg-white transition-colors shadow-sm">
                ...
                <div class="my-auto w-full">
                    <div class="mb-4">
                        <img src="${pageContext.request.contextPath}/img/transcake-03.png" alt="Transcake Logo"
                            class="h-10 w-auto object-contain mb-3 filter brightness-0">
                        <h2 class="text-3xl font-black text-slate-900 mb-1.5 font-['Playfair_Display']">Chào mừng bạn!
                        </h2>
                        <p class="text-slate-500 font-medium text-sm">Vui lòng đăng nhập hoặc đăng ký để tiếp tục trải
                            nghiệm.</p>
                    </div>
                    ...
                    <div class="flex gap-6 mb-4 border-b border-slate-200">
                        <button id="tabLogin"
                            class="pb-2.5 text-sm font-bold text-[#6200EE] border-b-2 border-[#6200EE] transition-colors">Đăng
                            nhập</button>
                        <button id="tabRegister"
                            class="pb-2.5 text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">Đăng
                            ký</button>
                    </div>
                    ...
                    <form id="registerForm" class="space-y-2.5 hidden">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Mã
                                OTP</label>
                            <input type="text" id="registerOtp" placeholder="Nhập mã OTP 6 số"
                                class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#6200EE]/50 focus:border-[#6200EE] transition-all text-sm font-medium placeholder:text-slate-400 text-center tracking-[0.5em] font-mono text-lg"
                                maxlength="6">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Mật
                                khẩu</label>
                            <input type="password" id="registerPassword" placeholder="Tạo mật khẩu"
                                class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#6200EE]/50 focus:border-[#6200EE] transition-all text-sm font-medium placeholder:text-slate-400">
                        </div>
                        <button type="button" id="registerBtn"
                            class="w-full mt-3 bg-[#FF6B00] text-white py-3 rounded-xl font-bold shadow-lg shadow-orange-500/30 hover:scale-[1.02] transition-transform">Đăng
                            ký ngay</button>
                        <p class="text-center text-[11px] font-medium text-slate-500 mt-2.5 leading-relaxed">Bằng việc
            ...
            <div class="w-full md:w-1/2 hidden md:block relative md:h-full">
                <img src="${pageContext.request.contextPath}/img/Gemini_Generated_Image_tji7owtji7owtji7.png"
                    class="absolute inset-0 w-full h-full object-cover">
                <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
                <div class="absolute bottom-12 left-12 right-12 text-white">
                    <span
                        class="inline-block px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-wider mb-4 border border-white/30">Nền
                        tảng số 1</span>
                    <h3 class="text-3xl font-black mb-3 font-['Playfair_Display']">Kết nối những chuyến đi</h3>
                    <p class="text-white/80 font-medium leading-relaxed">Cùng Transcake kiến tạo nên cộng đồng di chuyển
                        thông minh, an toàn và cá nhân hóa.</p>
                </div>
            </div>
